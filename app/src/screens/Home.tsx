...
function LeadsMock() {
  return (
    <div className="relative w-full">
      {/* Toast */}
      <div className="lm-ping-in flex items-center gap-2.5 bg-white border border-[#e8e3da] rounded-[14px] px-4 py-3 shadow-[0_8px_24px_-8px_rgba(20,22,18,0.12)] mb-3.5">
      ...
      {/* Card */}
      <div className="lm-float bg-white border border-[#e8e3da] rounded-[18px] overflow-hidden shadow-[0_4px_8px_-2px_rgba(20,22,18,0.06),0_32px_64px_-16px_rgba(20,22,18,0.20)]">
        {/* Browser chrome bar */}
        <div className="flex items-center gap-1.5 px-4 py-3 border-b border-[#e8e3da] bg-[#faf8f5]">
          <div className="w-2.5 h-2.5 rounded-full bg-[#f6c7c0]" />
          <div className="w-2.5 h-2.5 rounded-full bg-[#f6e2a4]" />
          <div className="w-2.5 h-2.5 rounded-full bg-[#b4d5c0]" />
          <div className="flex-1 mx-3 h-5 rounded-md bg-[#ece8e0] flex items-center px-2.5">
            <span className="text-[10px] text-[#9b9e96] font-medium tracking-tight">app.shortyharris.com</span>
          </div>
          <span className="text-[11px] font-bold bg-[#3c7a5b] text-white rounded-full px-2.5 py-[3px]">3 new</span>
        </div>
        {/* Header */}
        <div className="flex items-center justify-between px-[18px] py-[12px] border-b border-[#e8e3da]">
          <span className="text-[14px] font-bold text-[#1a1b17]">Hot Leads</span>
          <span className="text-[11px] text-[#9b9e96] font-medium">Today</span>
        </div>
        {MOCK_LEADS.map((l) => (
          <div key={l.name} className="flex items-start gap-3 px-[18px] py-[13px] border-b border-[#e8e3da] last:border-b-0 hover:bg-[#faf8f5] transition-colors cursor-pointer">
...
const STEPS = [
  {
    n: "01",
    title: "We find who to target",
    body: "We scan local directories and identify businesses that match your ideal customer — right location, right category, right size. No manual research needed.",
  },
  {
    n: "02",
    title: "AI writes it. You approve it.",
    body: "Personalized outreach is drafted by AI and reviewed in a queue before anything sends. Your brand stays sharp. Your message stays on point.",
  },
  {
    n: "03",
    title: "You only see the warm ones",
    body: "When a business replies with genuine interest, it hits your dashboard instantly — with full context and their contact details. Just show up and close.",
  },
];
  ...
  return (
    <section className="max-w-[1200px] mx-auto w-full px-6 lg:px-12 pb-24" id="how">
      ...
      >
        ...
        <motion.h2 variants={fadeUp} className="font-extrabold tracking-[-0.03em] text-[#1a1b17] m-0 mb-3" style={{ fontSize: "clamp(28px, 3vw, 40px)" }}>
          Three steps. Zero cold calls.
        </motion.h2>
        <motion.p variants={fadeUp} className="text-[16px] text-[#54574e] max-w-[460px] leading-[1.55] m-0">
          We handle the prospecting and outreach. You handle the conversations that actually matter.
        </motion.p>
      </motion.div>
